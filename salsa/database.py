...
# salsa /data/seld2022/features/salsa/foa/24000fs_512nfft_300nhop_5cond_9000fmaxdoa/
# melspec /data/seld2022/features/melspec/24000fs_512nfft_300nhop_128nmels
class Database:
    """
    Database class to handle one input streams for SELD
    """
    def __init__(self,
                 feature_root_dir: str = '/data/seld2022/features/salsa/foa/24000fs_512nfft_300nhop_5cond_9000fmaxdoa/dev-train-tau',
                 gt_meta_root_dir: str = '/data/seld2022/metadata_dev/',
                 audio_format: str = 'foa', n_classes: int = 13, fs: int = 24000,
                 n_fft: int = 512, hop_len: int = 300, label_rate: float = 10, train_chunk_len_s: float = 8.0,
        ...
        #print('max_nframes_per_file', self.max_nframes_per_file)


        assert audio_format in ['foa', 'mic'], 'Incorrect value for audio format {}'.format(audio_format)
        assert os.path.isdir(os.path.join(self.feature_root_dir, 'dev-train-tau')), '"dev" folder is not found'
        ...
        print('self.fs', self.fs, 'self.hop_len', self.hop_len, 'self.label_upsample_ratio', self.label_upsample_ratio)
        print('self.feature_rate', self.feature_rate, 'self.label_rate', self.label_rate)
    ...
    def get_segment_idxes(self, n_frames: int, downsample_ratio: int, pointer: int):
        ...
        print('n_crop_frames:',n_crop_frames, 'start, end, step', pointer, pointer + n_crop_frames - self.chunk_len // downsample_ratio + 1, self.chunk_hop_len // downsample_ratio)
    ...
    def get_split(self, split: str, split_meta_dir: str = '/data/seld2022/metadata_dev', stage: str = 'fit'):
        """
        ...
        :param split: Split of data, choices: 'train', 'val', 'test', 'eval': load chunk of data
        :param split_meta_dir:
            Directory where meta of split is stored.
        :param stage:
        ...
        dirname = f'dev-{split}-tau'
        split_sed_feature_dir = os.path.join(self.feature_root_dir, dirname)
        gt_meta_dir = os.path.join(self.gt_meta_root_dir, dirname)
        split_filenames = [filename[:-4] for filename in os.listdir(os.path.join('/data/seld2022/foa_dev', dirname))]
    ...
    def load_chunk_data(self, split_filenames: List, split_sed_feature_dir: str, gt_meta_dir: str):
        ...
        for filename in split_filenames:
            ...
            with h5py.File(feature_fn, 'r') as hf:
                ...
                # print('no channels', feature.shape[0])
                # Normalize feature
            ...
            n_frames = n_feature_frames #TODO: check
            ...
            # print('feature_pointer', feature_pointer)
            ...
            # print('gt_pointer', gt_pointer)

            if len(feature_idxes) != len(gt_idxes):
                # print('diff lengths', len(feature_idxes), len(gt_idxes))
                # import ipdb; ipdb.set_trace()
                feature_idxes = feature_idxes[:-1]
    ...
    def load_classwise_gt(self, gt_meta_fn, n_frames):
        ...
        # print('n_label_frames: n_frames:', n_frames, 'self.label_upsample_ratio:', self.label_upsample_ratio)
        ...
        if self.output_format in ['reg_xyz', 'accdoa']:
            ...
            # print('sed_target', sed_target.shape)
...
if __name__ == '__main__':
    tp_db = Database(feature_root_dir='/data/seld2022/features/salsa/foa/24000fs_512nfft_300nhop_5cond_9000fmaxdoa/',
                    gt_meta_root_dir='/data/seld2022/metadata_dev/',
                    output_format='reg_xyz',
                    test_chunk_len_s=8,
                    test_chunk_hop_len_s=0.5,
                    audio_format='foa')

    db_data = tp_db.get_split(split='train', split_meta_dir='/data/seld2022/', stage='fit')

    print("db_data['features'].shape", db_data['features'].shape)
    print("db_data['sed_targets'].shape", db_data['sed_targets'].shape)
    print("db_data['doa_targets'].shape", db_data['doa_targets'].shape)
    print('len1', len(db_data['feature_chunk_idxes']))
    print('len2', len(db_data['gt_chunk_idxes']))
    print('len3', len(db_data['filename_list']))
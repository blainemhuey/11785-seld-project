...
if __name__ == '__main__':
    # test dataloader
    db = Database(feature_root_dir='/data/seld2022/features/salsa/foa/24000fs_512nfft_300nhop_5cond_9000fmaxdoa/',
                  audio_format='foa', output_format='reg_xyz')
    data_db = db.get_split(
        split='train', split_meta_dir='/data/seld2022/metadata_dev')

    dataset = SeldDataset(db_data=data_db)
    print('Number of samples: {}'.format(len(dataset)))

    index = np.random.randint(len(dataset)) # just one sample
